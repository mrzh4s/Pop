...
class DBConnectionFactory
{
    ...
    private function createMySQLConnection($name, $config)
    {
        ...
        try {
            ...
            if (app_debug()) {
                error_log("MySQL connection '{$name}' established: {$host}:{$port}/{$database}");
            }
    ...
    private function createSQLServerConnection($name, $config)
    {
        ...
        try {
            ...
            if (app_debug()) {
                error_log("SQL Server connection '{$name}' established: {$host}:{$port}/{$database}");
            }
    ...
    private function createSQLiteConnection($name, $config)
    {
        ...
        try {
            ...
            if (app_debug()) {
                error_log("SQLite connection '{$name}' established: {$dbPath}");
            }
            ...
            error_log("SQLite connection '{$name}' failed: " . $e->getMessage());
    ...
    private function recordConnection($name, $attempt, $time, $success, $error = null)
    {
        ...
        if ($success && $time > 200 && app_debug()) {
            error_log("SLOW DB CONNECTION '{$name}': {$time}ms on attempt {$attempt}");
        }

        if (!$success && app_debug()) {
            error_log("DB connection '{$name}' attempt {$attempt} failed: {$error}");
        }
    }
...
class DB
{
    ...
    {
        return DBConnectionFactory::getInstance()->getAvailableConnections();
    }
    ...
    {
        return DBConnectionFactory::getInstance()->getConnectionConfig($name);
    }
