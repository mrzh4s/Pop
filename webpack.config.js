...
const JavaScriptObfuscator = require('webpack-obfuscator'); // Added for obfuscation
const fs = require("fs");
...
class FilesHandler {
  ...
  getMergeFiles(config) {
    ...
    Object.entries(clonedEntry).forEach(([key, values]) => {
      values.forEach((value) => {
        ...
        const dist = value.dist.replace("apps/assets", "");
  ...
  getAppEntries() {
    ...
    const appPath = path.resolve("src/js");
...
module.exports = (env) => {
  return {
    ...
    output: {
      filename: "js/[name].js",
      path: path.resolve(__dirname, "./" + vendorConfig.output),
      library: {
        type: "umd",
      },
    },
    ...
    plugins: [
      ...
      // JavaScript Obfuscator - Only in production with DEBUG
      ...(env.production ? [
        new JavaScriptObfuscator({
          // Enable logging to see what files are being processed
          log: true,
        ...
        // NO FILE PATTERN - Let it process ALL js files by default
      ] : [])
    ],
    target: ["web", "es5"],
    optimization: {
